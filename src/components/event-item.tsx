...
import { getBorderRadiusClasses, getEventColorClasses } from "@/components/utils";
  ...
  if (view === "month") {
    return (
      <EventWrapper
        ...
        onClick={onClick}
        className={cn("mt-[var(--event-gap)] h-[var(--event-height)] items-center text-[10px] sm:text-[13px]", className)}
      ...
      >
        {durationMinutes < 45 ? (
          ...
          <>
            ...
            {showTime && <div className="truncate font-normal opacity-70 sm:text-xs uppercase">{getEventTime()}</div>}
          </>
        )}
      </EventWrapper>
    );
